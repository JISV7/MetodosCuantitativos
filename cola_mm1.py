class MM1Queue:
    ...
    def clientes_promedio_cola(self):
        ...
        return (self.tasa_llegada ** 2) / (self.tasa_servicio * (self.tasa_servicio - self.tasa_llegada))

    def clientes_promedio_sistema(self):
        """Número promedio de clientes en el sistema (L)."""
        return self.tasa_llegada / (self.tasa_servicio - self.tasa_llegada)
    ...
    def tiempo_espera_sistema(self):
        ...
        return 1 / (self.tasa_servicio - self.tasa_llegada)

    def probabilidad_vacio(self):
        """Probabilidad de que el sistema esté vacío (P0)."""
        # Fórmula para M/M/1: P0 = 1 - ρ
        return 1 - self.utilizacion


def presentar_resultados(cola: MM1Queue):
    """Presenta los resultados del sistema M/M/1."""
    ...
    print(f"Probabilidad de que el sistema esté vacío (P0): {cola.probabilidad_vacio():.4f}")
...
if __name__ == "__main__":
    try:
        ...
        # Crear una instancia del sistema M/M/1
        cola_mm_1 = MM1Queue(llegadas_totales, tiempo_observacion, atendidos_totales, tiempo_servicio)

        # Mostrar los resultados
        presentar_resultados(cola_mm_1)
