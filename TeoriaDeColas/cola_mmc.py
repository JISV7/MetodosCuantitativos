...
class MMCQueue:
    def __init__(
        ...
        if self.utilizacion >= 1:
            raise ValueError(
                "El sistema no es estable (ρ ≥ 1). La tasa de llegada debe ser menor que la tasa de servicio multiplicada por el número de servidores."
            )

        # Validación para evitar ZeroDivisionError en cálculos posteriores
        if 1 - self.utilizacion <= 0:
            raise ValueError(
                "La diferencia (1 - ρ) debe ser mayor que 0 para evitar división por cero."
            )
    ...
    def probabilidad_sistema_vacio(self):
        ...
        sumatoria = sum(
            [
                (self.tasa_llegada / self.tasa_servicio) ** n / math.factorial(n)
                for n in range(self.servidores)
            ]
        )
        segundo_termino = (
            (self.tasa_llegada / self.tasa_servicio) ** self.servidores
        ) / (math.factorial(self.servidores) * (1 - self.utilizacion))
    ...
    def clientes_promedio_cola(self):
        ...
        Lq = (
            ...
            * (self.tasa_llegada / self.tasa_servicio) ** self.servidores
            * self.utilizacion
        ) / (math.factorial(self.servidores) * (1 - self.utilizacion) ** 2)
        return Lq
...
def presentar_resultados(cola: MMCQueue):
    """Presenta los resultados del sistema M/M/c."""
    print(f"Tasa de llegada (λ): {cola.tasa_llegada:.4f} clientes por unidad de tiempo")
    print(
        f"Tasa de servicio (μ): {cola.tasa_servicio:.4f} clientes atendidos por servidor por unidad de tiempo"
    )
    print(f"Utilización del sistema (ρ): {cola.utilizacion:.4f}")
    print(
        f"Probabilidad de que el sistema esté vacío (P0): {cola.probabilidad_sistema_vacio():.4f}"
    )
    print(
...
if __name__ == "__main__":
    try:
        ...
        # Crear una instancia del sistema M/M/c
        cola_mm_c = MMCQueue(
            llegadas_totales,
            tiempo_observacion,
            atendidos_totales,
        ...
        presentar_resultados(cola_mm_c)
